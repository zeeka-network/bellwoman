...
/// Distributes powers of g over elements
KERNEL void distribute_powers(GLOBAL FIELD* elements,
                                uint n,
                                uint deg,
                                FIELD g) {
  const uint gid = GET_GLOBAL_ID() << deg;
  const uint len = 1 << deg;
  FIELD curr = FIELD_pow(g, gid);
  for(uint i = 0; i < len; i++) {
    elements[gid+i] = FIELD_mul(elements[gid+i], curr);
    curr = FIELD_mul(curr, g);
  }
}

/// Element-wise mul
KERNEL void mul_assign(GLOBAL FIELD* a,
                        GLOBAL FIELD* b,
                        uint n) {
  const uint gid = GET_GLOBAL_ID();
...
/// Element-wise sub
KERNEL void sub_assign(GLOBAL FIELD* a,
                        GLOBAL FIELD* b,
                        uint n) {
  const uint gid = GET_GLOBAL_ID();
...
/// Unmont
KERNEL void unmont(GLOBAL FIELD* a) {
  const uint gid = GET_GLOBAL_ID();
  a[gid] = FIELD_unmont(a[gid]);
}
