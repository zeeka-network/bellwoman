...
DEVICE bool FIELD_get_bit(FIELD l, uchar i) {
...
DEVICE ushort FIELD_get_bits(FIELD l, uchar skip, uchar window) {
  ushort ret = 0;
  for(uchar i = 0; i < window; i++) {
    ret <<= 1;
...
DEVICE FIELD FIELD_mul_3b(FIELD a) {
  FIELD a2 = FIELD_double(a);
  FIELD a4 = FIELD_double(a2);
  FIELD a12 = FIELD_add(FIELD_double(a4), a4);
  return a12;
}
